...
void Logger::writerThread() {
    ...
    while (true) {
        ...
        while (!m_queue.empty()) {
            ...
            if (m_file.is_open()) {
                // P0-crash-fix: to_bytes throws on invalid UTF-16 in log lines.
                // Logging must never crash; fall back to raw byte copy.
                std::string utf8Line;
                try {
                    utf8Line = std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(line);
                } catch (...) {
                    utf8Line.assign(line.begin(), line.end());
                }
                m_file << utf8Line << std::endl;
            }
            if (m_consoleOutput) {
