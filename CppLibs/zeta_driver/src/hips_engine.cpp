...
void HipsEngine::loadDefaultRules() {
    ...
    DefaultRule defaults[] = {
        ...
        {L"def_file_idea", 2001, L"pycharm64.exe",       L"", HIPS_ALLOW, false, 0},
...
HipsAction HipsEngine::matchRule(unsigned long code, const std::wstring& process,
    ...
    for (const auto& r : m_rules) {
        if (r.code != code) continue;
        if (!r.process.empty() && process.find(r.process) != std::wstring::npos) {
            m_lastMatchedScore = r.score;
            return r.action;
        }
        if (!r.target.empty() && target.find(r.target) != std::wstring::npos) {
            m_lastMatchedScore = r.score;
            return r.action;
        }
    }
