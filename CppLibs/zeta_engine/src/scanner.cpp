...
static int yara_callback(YR_SCAN_CONTEXT* context, int message, void* message_data, void* user_data) {
    ...
    if (message == CALLBACK_MSG_RULE_MATCHING) {
        ...
        if (rule && ctx) {
            ...
            if (rule->identifier) {
                try {
                    ctx->matchedRule = std::wstring_convert<std::codecvt_utf8<wchar_t>>().from_bytes(rule->identifier);
                } catch (...) {
                    ctx->matchedRule.clear();
                }
            } else {
                ctx->matchedRule.clear();
            }
...
bool YaraScanner::scanMem(const BYTE* data, size_t size,
    ...
    // yr_rules_scan_mem 扫描内存缓冲；timeout=0 表示无超时
...
ScanResult ScanEngine::scanFile(const std::wstring& filePath, bool enhancedMode) {
    ...
    if (enabled.enableYara) {
        ...
        if (m_yaraScanner.scan(filePath, ruleName, label, isHigh)) {
            ...
            result.disposition = isHigh ? ScanResult::QUARANTINE : ScanResult::NONE;
            std::lock_guard<std::mutex> lock(m_mutex);
            if (m_cache.size() < MAX_CACHE) m_cache[filePath] = result;
            return result;
        }
    ...
    while (VirtualQueryEx(hProc, addr, &mbi, sizeof(mbi)) == sizeof(mbi)) {
        if ((mbi.State & MEM_COMMIT) &&
            ...
            if (ReadProcessMemory(hProc, mbi.BaseAddress, buf.data(), regionSize, &bytesRead) &&
                ...
                bool isHigh = false;
                if (m_yaraScanner.scanMem(buf.data(), bytesRead, rule, label, isHigh)) {
                    result.type = L"mem-yara";
                    result.status = isHigh ? L"Threat" : L"Suspicious";
                    result.detail = L"pid=" + std::to_wstring(pid) + L" " + rule;
                    result.isHigh = isHigh;
                    found = TRUE;
                    break;  // 首个命中即返回
                }
