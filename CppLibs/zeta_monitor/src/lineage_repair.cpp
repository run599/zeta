...
bool SystemRepair::repair(const std::wstring& type) {
    ...
    // "all": 逐个调用各修复函数 (内部自行判断是否需要修)
    if (type == L"all" || type.empty()) {
        int fixed = 0;
        if (repairMbr()) fixed++;
        if (repairRestrict()) fixed++;
        if (repairFileType()) fixed++;
        if (repairFileIcon()) fixed++;
        if (repairImageHijack()) fixed++;
        if (repairWallpaper()) fixed++;
        Logger::instance().info(L"Repair", L"Repair", L"All repair done, fixed=" + std::to_wstring(fixed));
        return fixed > 0;
    }
...
bool SystemRepair::repairMbr() {
    // 保守版 MBR 修复：仅当 55AA 引导签名被破坏时补写签名，不修改分区表。
    // 写物理盘需要管理员权限，打开失败则返回 false（标记需手动处理）。
    HANDLE hDrive = CreateFileW(L"\\\\.\\PhysicalDrive0",
        GENERIC_READ | GENERIC_WRITE,
        FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
    if (hDrive == INVALID_HANDLE_VALUE) {
        Logger::instance().warn(L"Repair", L"MBR",
            L"MBR repair requires administrator elevation (open failed)");
        return false;
    }
    ...
    if (!ok || bytesRead < 512) {
        CloseHandle(hDrive);
        Logger::instance().warn(L"Repair", L"MBR", L"MBR read failed");
        return false;
    }
    ...
    BOOL wrote = WriteFile(hDrive, mbr, 512, &bytesWritten, nullptr);
    CloseHandle(hDrive);

    if (wrote && bytesWritten == 512) {
        Logger::instance().warn(L"Repair", L"MBR", L"MBR boot signature restored (55AA)");
        return true;
    }
    Logger::instance().warn(L"Repair", L"MBR", L"MBR signature write failed");
    return false;
}
