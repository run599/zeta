...
// ETW Monitor
// ============================================================
EtwMonitor::EtwMonitor() : m_running(false), m_sessionHandle(0), m_traceHandle(0) {}

EtwMonitor::~EtwMonitor() { stop(); }

// P1-4: Microsoft-Windows-Kernel-Process provider GUID
// 提供 ProcessStart (EventId=1) / ProcessStop (EventId=2) / DCStart (EventId=3)
// 等结构化事件，比旧版 KERNEL_LOGGER 的 EVENT_TRACE_FLAG_PROCESS 更可靠，
// 且能捕获生命周期 < 500ms 的短命进程。
static const GUID g_KernelProcessGuid = {
    0x22FB2CD6, 0x0E7B, 0x422B, {0xA0, 0xC7, 0x2F, 0xAD, 0x1F, 0xD0, 0xE7, 0x16}
};
static const WCHAR g_ZetaEtwSession[] = L"ZETA_ProcTrace";

bool EtwMonitor::start() {
    Logger::instance().info(L"Monitor", L"ETW", L"Starting Kernel-Process ETW session");

    // 1. 创建专用 ETW session
    ULONG bufSize = sizeof(EVENT_TRACE_PROPERTIES) +
                    (ULONG)(wcslen(g_ZetaEtwSession) + 1) * sizeof(WCHAR);
    ...
    wcscpy_s((LPWSTR)((BYTE*)props + props->LoggerNameOffset),
             (bufSize - sizeof(EVENT_TRACE_PROPERTIES)) / sizeof(WCHAR),
             g_ZetaEtwSession);

    ULONG status = StartTraceW(&m_sessionHandle, g_ZetaEtwSession, props);
    if (status == ERROR_ALREADY_EXISTS) {
        // 已有同名 session（可能是上次异常退出残留），先停再建
        ControlTraceW(0, g_ZetaEtwSession, props, EVENT_TRACE_CONTROL_STOP);
        status = StartTraceW(&m_sessionHandle, g_ZetaEtwSession, props);
    }
    ...
    // 2. 启用 Kernel-Process provider（仅进程事件，关掉线程/映像以减少噪声）
    //    不设 EnableProperty（PID/TID 从事件 UserData 直接解析，无需附加属性）
    ENABLE_TRACE_PARAMETERS params = {0};
    params.Version = ENABLE_TRACE_PARAMETERS_VERSION_2;
    status = EnableTraceEx2(m_sessionHandle, &g_KernelProcessGuid,
                            EVENT_CONTROL_CODE_ENABLE_PROVIDER,
                            TRACE_LEVEL_INFORMATION, 0, 0, 0, &params);
    if (status != ERROR_SUCCESS) {
        Logger::instance().error(L"Monitor", L"ETW",
            L"EnableTraceEx2 failed: " + WinHelpers::formatError(status));
        ControlTraceW(m_sessionHandle, g_ZetaEtwSession, nullptr, EVENT_TRACE_CONTROL_STOP);
        m_sessionHandle = 0;
        return false;
    }
    ...
    logfile.LoggerName = const_cast<wchar_t*>(g_ZetaEtwSession);
    ...
    Logger::instance().info(L"Monitor", L"ETW", L"Kernel-Process ETW session started");
    return true;
}
...
// 解析 Kernel-Process 事件中的字符串属性 (ImageFileName / CommandLine)
// 利用 TdhGetProperty 按属性名提取，避免硬编码偏移导致的版本不兼容。
static std::wstring EtwGetStringProp(PEVENT_RECORD pEvent, const wchar_t* name) {
    ...
    ULONG size = 0;
    // 第一次调用获取所需缓冲区大小
    if (TdhGetProperty(pEvent, 0, nullptr, 1, &desc, 0, nullptr) != ERROR_INSUFFICIENT_BUFFER)
        return L"";
    if (TdhGetPropertySize(pEvent, 0, nullptr, 1, &desc, &size) != ERROR_SUCCESS)
        return L"";

    std::vector<wchar_t> buf(size / sizeof(wchar_t) + 1);
    if (TdhGetProperty(pEvent, 0, nullptr, 1, &desc, size, (PBYTE)buf.data()) != ERROR_SUCCESS)
        return L"";
...
void WINAPI EtwMonitor::eventRecordCallback(PEVENT_RECORD pEvent) {
    ...
    EtwMonitor* self = reinterpret_cast<EtwMonitor*>(pEvent->UserContext);
    if (!self) return;

    // 只处理 Kernel-Process provider 的事件
    if (pEvent->EventHeader.ProviderId != g_KernelProcessGuid)
        return;

    USHORT eid = pEvent->EventHeader.EventDescriptor.Id;
    // EventId: 1=ProcessStart, 2=ProcessStop, 3=DCStart(启动期存量), 4=DCEnd
    bool isStart = (eid == 1 || eid == 3);
    bool isStop  = (eid == 2 || eid == 4);
    if (!isStart && !isStop) return;

    // 提取 PID / PPID（固定整型字段，位于事件头部 UserData 起始）
    // Kernel-Process ProcessStart 结构: ProcessId(ULONG), ParentId(ULONG), ...
    ULONG pid = 0, ppid = 0;
    if (pEvent->UserDataLength >= sizeof(ULONG) * 2) {
        const ULONG* u = reinterpret_cast<const ULONG*>(pEvent->UserData);
        pid = u[0];
        ppid = u[1];
    }
    if (pid == 0) return;

    std::wstring name = EtwGetStringProp(pEvent, L"ImageFileName");
    if (name.empty()) name = L"<unknown>";

    if (self->m_callback) {
        self->m_callback(pid, ppid, name, isStart);
    }
}
...
__declspec(dllexport) void zeta_monitor_start_etw() {
    // P1-4: 把 Kernel-Process ETW 事件汇入与轮询监控相同的 g_newProcCb 管道，
    // 实现"内核回调 + ETW"双源合并（isStart=false 的停止事件也上报，便于清理）。
    // 注意：path 必须传合法 wchar_t*（空串），不能传 nullptr —— onNewProcessCreated
    // 会对 path 做 std::wstring(path) 构造，nullptr 会在 ucrtbase 触发 0xC0000005。
    static const wchar_t emptyPath[] = L"";
    g_etwMon.setCallback([](unsigned long pid, unsigned long ppid,
        const std::wstring& name, bool isStart) {
        if (g_newProcCb) {
            g_newProcCb(pid, ppid, name.c_str(), emptyPath);
        }
    });
    g_etwMon.start();
}
