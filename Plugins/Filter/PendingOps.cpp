...
NTSTATUS PendOperation(PFLT_CALLBACK_DATA Data, PCFLT_RELATED_OBJECTS FltObjects,
    ...
    // P1-1: 挂起待决操作开关 - 关闭时不进入挂起状态
    // 调用方 (PreCreate/PreWrite 等) 收到失败后应直接放行 (不阻断)
    if (!g_SuspendEnabled) {
        ZETA_INFO("PendOperation SKIPPED - suspend disabled (PID=%lu Code=%lu)\n", ProcessId, MessageCode);
...
// ============================================================================
// CompleteAllPendingOperations — 先 FltCompletePendedPreOperation 再释放
//
// 在 DriverUnload 中、FltUnregisterFilter 之前调用。
// 与 CleanupAllPendingOperations 不同：此函数先完成待决操作（Deny 所有）
// 再释放 PENDING_OP 内存，避免 FltUnregisterFilter 时 FLTMGR 访问悬空 CallbackData。
// ============================================================================
VOID CompleteAllPendingOperations() {
    if (!g_PendingOpsInitialized) return;

    DbgPrint("ZETA: Completing all pending operations (deny all)\n");
    ...
    // Complete each pending op with deny, then free
    for (ULONG i = 0; i < count; i++) {
        ...
        if (op->CallbackData) {
            __try {
                ...
                DbgPrint("ZETA: CompleteAllPendingOps - COMPLETED (deny) PID=%lu Code=%lu\n",
                    op->ProcessId, op->MessageCode);
            }
            __except (EXCEPTION_EXECUTE_HANDLER) {
                DbgPrint("ZETA: CompleteAllPendingOps - exception completing CallbackData (PID=%lu)\n",
                    op->ProcessId);
            }
        }
    ...
    DbgPrint("ZETA: Pending ops completed and cleaned up\n");
}
