...
void NavSidebar::setLightMode(bool light) {
    if (m_isLight != light) {
        m_isLight = light;
        update();
    }
...
void NavSidebar::drawBackground(QPainter& p) {
    if (m_isLight) {
        // 亮色: 近白侧栏 + 细分隔线 (与新 Design Token 对齐)
        p.fillRect(rect(), QColor(245, 244, 248, 255));
        p.setPen(QColor(220, 219, 227));
        p.drawLine(width() - 1, 0, width() - 1, height());
    } else {
        // 暗色: 半透明背景透出毛玻璃 (macOS 风格)
        p.fillRect(rect(), QColor(18, 20, 29, 140));
    }
}
...
void NavSidebar::drawLogo(QPainter& p) {
    ...
    p.setPen(Qt::NoPen);
    p.setBrush(m_isLight ? QColor(255, 122, 0) : cAccent);
    ...
    p.setFont(titleFont);
    p.setPen(m_isLight ? QColor(31, 31, 41) : cText);
    ...
    p.setFont(subFont);
    p.setPen(m_isLight ? QColor(107, 107, 120) : cTextDim);
...
void NavSidebar::drawItem(QPainter& p, int idx, bool selected,
    ...
    // macOS 风格: 圆角胶囊高亮 (暗色靛蓝 / 亮色橙)
    p.setPen(Qt::NoPen);
    QRect cap = ir.adjusted(8, 2, -8, -2);   // 胶囊区域 (留边距)
    if (selected) {
        p.setBrush(m_isLight ? QColor(255, 122, 0, 26) : QColor(99, 102, 241, 70));
        p.drawRoundedRect(cap, 10, 10);
    } else if (pressed) {
        p.setBrush(m_isLight ? QColor(230, 228, 236) : cBgPressed);
        p.drawRoundedRect(cap, 10, 10);
    } else if (hovered) {
        p.setBrush(m_isLight ? QColor(255, 122, 0, 16) : QColor(99, 102, 241, 28));
        p.drawRoundedRect(cap, 10, 10);
    }
...
void NavSidebar::drawItemIcon(QPainter& p, const QPainterPath& path,
    ...
    p.setPen(Qt::NoPen);
    if (m_isLight) {
        p.setBrush(selected ? QColor(255, 122, 0) : QColor(107, 107, 120));
    } else {
...
void NavSidebar::drawItemLabel(QPainter& p, const QString& text,
    ...
    p.setFont(f);
    if (m_isLight) {
        p.setPen(selected ? QColor(31, 31, 41) : QColor(107, 107, 120));
    } else {
